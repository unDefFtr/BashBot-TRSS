...
logger.info(
    logger.yellow(
        `[${AppName}] 共加载了 ${loadedFilesCount} 个函数 ${loadedFilesCounterr} 个失败 耗时 ${endTime - startTime}ms。`,
    ),
);
logger.info(logger.yellow(`欢迎使用 BashBot！`));
export { apps };
            ...
            jsFilePaths.map(async (item) => {
                try {
                    ...
                    for (const key of Object.keys(allExport)) {
                        if (
                            typeof allExport[key] === "function" &&
                            allExport[key].prototype
                        ) {
                            let className = key;
                            if (
                                ...
                                logger.info(
                                    `[${AppName}] 同名导出 ${key} 重命名为 ${className} : ${item}`,
                                );
                            }
                    ...
                    logger.error(
                        `[${AppName}] 加载 ${item} 文件失败: ${error.message}`,
                    );
                    loadedFilesCounterr++;
                }
            }),
